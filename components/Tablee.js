...
import { Button,Divider,Grid,Typography,styled,useTheme } from '@mui/material';
...
export default function EnhancedTable({rows, headCells, headTitle, addIcon=false, screenIcon=false, deleteUser=false,isCheckbox=false}) {
  ...
  const handlePage = (page) => {setPage(page);console.log("11111111111",page);};
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Box sx={{ width: '100%', mb: 2 }}>
        <Box display="flex" py="24px" pr="24px" sx={{backgroundColor: theme.palette.primary.main }}>{headTitle}</Box>
        <Divider/>
        <EnhancedTableToolbar handlePageSizeChange={handlePageSizeChange} numSelected={selected.length}  theme ={theme} addIcon={addIcon} screenIcon={screenIcon} deleteUser={deleteUser} pageContent={pageContent}/>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right" >
                      <Box display="flex">
                      {row.manager.image} 
                      <Box display="column" pr={1}>
                        <Typography variant='h6'>
                      {row.manager.name}

                        </Typography>
                        <Typography variant='subtitle2' color="text.gray" fontWeight={400}>
                          کد ملی:
                    ...
                    {row.actions.trash} {row.actions.view}
                      </Box></TableCell>
